...
어려운 문제라기 보단, 까다롭고 귀찮은 문제였다. <br>
카카오는 disjoint set 문제를 상당히 좋아하는 것 같다. <br>
다만 상당히 잘 만든 문제라고 느꼈는데,  union find를 정확히 이해하고 활용해야만 풀 수 있었다. <Br>
        단순히 코드만을 외워서는 어려웠을 것이다. <br>
그 이유는 연결을 끊어주는 연산의 존재와, 값을 업데이트 해주는 과정에서, 모든 변화들을 노드 집합 전체에 적용해 줘야 한다는 점이 어려웠다. <br>
이런 구현을 위해선 union과 find를 정확히 이해하고 응용할 줄 알아야 했다. <br>
결국 정말 느린 방법이지만, 변화가 생긴 cell의 부모를 알아낸 다음 같은 부모를 공유하는 모든 cell에 변화를 적용해 주었다. <br>
        물론 이런 방식은 경로 압축을 통해, 한 set의 모든 노드들이 같은 부모를 가르키도록 구현한 덕에 가능한 방법이였다.

### 성능 요약

메모리: 90.6 MB, 시간: 60.23 ms
