...
network-flow 기본 문제. 최대 유량 개념을 이용한 매우 기본적인 문제이다. <br>
파이프는 병렬적으로 있을 때 두 파이프 크기의 합만큼, 직렬로 있을 때, 더 작은 크기 만큼의 유량이 흐를 수 있다. 그냥 대놓고 최대 유량 문제 <br>
최대 유량 개념을 배운 블로그에서 예시로 보여준 문제라, 직접 정석적인 에드몬드-카프 알고리즘을 짜볼 수 있는 기회였다. <br>
다만, 문제에서 함정으로 같은 파이프 입력이 여럿일 수 있었다. 그래서 파이프 용량에 대한 입력을 단순히 받아서 대입하는 것이 아니라, 용량을 계속해서 더해줘야 했다. 이 때문에 계속 틀렸었다. <br>
계속해서 bfs를 돌리며, 아직 물을 흘려보낼 수 있는 파이프를 반복적으로 찾아내며, 물을 흘려준다. <br>
더 이상 물을 흘릴 수 있는 파이프가 없다면 그 양이 최대 유량이 된다.
