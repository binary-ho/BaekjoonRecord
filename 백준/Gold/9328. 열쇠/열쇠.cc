...
void bfs() {
    int y_now, x_now, new_y, new_x, phase = 0;
    ...
    while(phase <= key_cnt + 1) {
        for(pair<int, int> window : windows) {
            ...
            que.push({window.first, window.second});
        }
        while(!que.empty()) {
            y_now = que.front().first;
            x_now = que.front().second;
            
            que.pop();
            for(int i = 0; i < 4; i++) {
                new_y = y_now + dy[i]; new_x = x_now + dx[i];
                if(0 > new_y || new_y >= h || 0 > new_x || new_x >= w) continue;
                if(map[new_y][new_x] == '*' || check[new_y][new_x][phase]) continue;
                if('A' <= map[new_y][new_x] && map[new_y][new_x] <= 'Z' && !keyCheck[map[new_y][new_x] - 'A']) continue;

                // 이제 가능한 부분
                check[new_y][new_x][phase] = true;
