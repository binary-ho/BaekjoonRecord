...
완전 그리디한 문제 <br>
두 파일을 합치면 새로운 크기를 가진 파일이 나오는데, 항상 가장 작은 두 파일의 합을 구하면 최소 합이 나온다. <Br>
그러면 가장 작은 파일 두개를 합쳐 새로운 파일을 만든 다음, 그 새로운 파일을 포함한 모든 파일 중 가장 작은 두 파일을 또 합치면 된다. <Br>
이런 연산의 무한 반복이므로, 가장 작은 두 파일을 빠르게 알아야하고, 새로운 파일이 가장 작은지 빠르게 확인할 수 있어야 한다. <br>
그래서 우선순위 큐를 사용해서 구했다! <br>
우선순위 큐를 사용해서 가장 작은 2개를 구해 더한 다음, 또 우선순위 큐에 넣어주면 O(logN)만에 정렬된다. <br>


### 성능 요약

메모리: 14324 KB, 시간: 1248 ms
